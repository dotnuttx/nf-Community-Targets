...
//        CLR_Debug::Printf("Loading Deployment Assemblies.\r\n");
    ...
    ~Settings()
    {
        ...
        int mSize = m_commands.size();

        for (int i = 0; i < mSize; i++)
        {
            Command* cmd = m_commands.front();
            int cmdSize = cmd->m_params.size();

            for (int j = 0; j < cmdSize; j++)
            {
                delete cmd->m_params.back();
                cmd->m_params.pop_back();
            }
    ...
    struct Command_Call : CLR_RT_ParseOptions::Command
    {
        ...
        Command_Call(Settings &parent, FPN call, const char *szName, const char *szDescription)
...
#define PARAM_EXTRACT_STRING(lst, index) ((CLR_RT_ParseOptions::Parameter_Generic *)(*lst)[index])->m_data.c_str()
    ...
    void BuildOptions()
    {
        ...
        OPTION_CALL(Cmd_Load, "-load", "Loads an assembly formatted for nanoCLR");
        PARAM_GENERIC("<file>", "File to load");

        OPTION_CALL(Cmd_LoadDatabase, "-loadDatabase", "Loads a set of assemblies");
        PARAM_GENERIC("<file>", "Image to load");

        OPTION_CALL(Cmd_Resolve, "-resolve", "Tries to resolve cross-assembly references");
    }

    HRESULT CheckAssemblyFormat(CLR_RECORD_ASSEMBLY *header, const char *src)
    {
        NANOCLR_HEADER();
    ...
    HRESULT Cmd_Load(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        ...
        const char *szName = PARAM_EXTRACT_STRING(params, 0);
    ...
    HRESULT Cmd_LoadDatabase(CLR_RT_ParseOptions::ParameterList *params = NULL)
    {
        ...
        {
            const char *szFile = PARAM_EXTRACT_STRING(params, 0);
